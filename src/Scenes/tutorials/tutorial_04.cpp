...
bool TutorialApp_04::Initialize() {
...
void TutorialApp_04::Run() {
    GLApp::Run();
}

void TutorialApp_04::Draw() {
    vbo->BufferData();
    ibo->BufferData();
...
void TutorialApp_04::OnMouseDown(int button, double xpos, double ypos) {
...
void TutorialApp_04::OnMouseUp(int button, double xpos, double ypos) {
    selected_point = NULL;
}

void TutorialApp_04::OnMouseMove(double xpos, double ypos) {
    xpos = (xpos / width) * 2 - 1;
    ypos = -(ypos / height) * 2 + 1;
...
Point* TutorialApp_04::ClosestPoint(float x, float y) {
    Point cursor = {x, y};
